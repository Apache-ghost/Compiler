"""
Modern PowerPoint Presentation Generator with Transitions
Creates a captivating, professional presentation with modern design and smooth transitions
"""
...
def add_slide_transition(slide, transition_type="push", duration=0.5):
    ...
    try:
        ...
        trans.set('spd', 'med')  # Speed: fast, med, slow
        trans.set('advTm', str(int(duration * 1000)))
        
        # Add specific transition type
        if transition_type == "push":
            push = OxmlElement('p:push')
            push.set('dir', 'l')  # Direction: l=left, r=right, u=up, d=down
            trans.append(push)
            ...
            zoom = OxmlElement('p:zoom')
            zoom.set('dir', 'in')
            trans.append(zoom)
        
        sld.insert(0, trans)
    except:
        pass  # If transition fails, continue without it
...
def add_modern_title_slide(prs, title, subtitle):
    ...
    add_gradient_background(
        slide, prs,
        RGBColor(10, 25, 47),  # Dark blue
        RGBColor(20, 66, 114)  # Medium blue
    )
    ...
    subtitle_frame.paragraphs[0].font.color.rgb = RGBColor(255, 193, 7)  # Amber accent
    ...
    add_slide_transition(slide, "fade", 1.0)
    return slide
...
def add_section_divider(prs, title, icon="", accent_color=RGBColor(255, 193, 7)):
    ...
    add_slide_transition(slide, "zoom", 0.8)
    return slide

def add_modern_content_slide(prs, title, content_points, icon="", bg_theme="light"):
    """Add a modern content slide with cards and icons"""
    slide_layout = prs.slide_layouts[6]
    slide = prs.slides.add_slide(slide_layout)
    ...
    # Modern title bar with rounded corners
    ...
    add_slide_transition(slide, "push", 0.7)
    return slide
...
def add_two_column_modern(prs, title, left_title, left_items, right_title, right_items):
    ...
    title_shape.fill.solid()
    title_shape.fill.fore_color.rgb = RGBColor(99, 102, 241)  # Indigo
    title_shape.line.fill.background()
    ...
    left_card.line.color.rgb = RGBColor(59, 130, 246)  # Blue
    ...
    right_card.line.color.rgb = RGBColor(168, 85, 247)  # Purple
    ...
    add_slide_transition(slide, "wipe", 0.7)
    return slide

def create_modern_presentation():
    """Create an enhanced modern presentation"""
    prs = Presentation()
    prs.slide_width = Inches(10)
    prs.slide_height = Inches(7.5)
    
    print("Creating modern presentation with transitions...")
    ...
    print("✓ Why it matters slide")
    
    # Slide 5: Languages Divider
    ...
    add_modern_content_slide(
        ...
        [
            ...
            "3️⃣ Syntactic Parser - Structure validation & parse trees",
            "4️⃣ Complete Analyzer - End-to-end processing",
            "5️⃣ GUI Interface - Interactive visualization tool"
        ],
        "🏗️"
    )
    print("✓ Components slide")
    
    # Slide 9: Features Divider
    ...
    print("✓ Token categories slide")
    
    # Slide 12: Examples Divider
    ...
    # Save
    filename = "Yaounde_Analyzer_Modern_Presentation.pptx"
    prs.save(filename)
    
    print(f"\n{'='*60}")
    print(f"✅ Modern presentation created successfully!")
    ...
    print(f"   • Rounded corners and shadows")
    print(f"✨ Ready for an amazing presentation!")
    print(f"{'='*60}\n")
